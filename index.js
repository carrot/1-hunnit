...
controller.on('channel_leave', function (bot, message) {
  if (message.user === process.env.ADMIN3) {
    bot.reply(message, 'LOL i\'ll bet my power source he will back. He always comes back like that person you are stuck in a love/hate relationship with. Its complicated. :peace_symbol: :wave: <@' + message.user + '> aka \'The Wanderer\'')
  } else {
    bot.reply(message, 'another one bites the dust. :peace_symbol: :wave: <@' + message.user + '>')
  }
})
...
function react (bot, message) {
  ...
  if (message.user === process.env.ADMIN13) {
    if (Math.random() < 0.1) {
      rxn.push('bagel', 'santa')
    }
  }
  ...
  if (validUsers.indexOf(message.user) !== -1) {
    ...
    var messageChecks = [
      ...
      ['WHO IS MATT SLITER?', 'basketball', 'The crossover king. He\'s only here so he don\'t get fined.'],
      ['WHO IS REHMAT QADIR?', 'gun', 'Not to front, he is someone who always keeps it :100: and he lives in a VR/AR world unlike you plebians :100:'],
      ['WHO IS BIG ELVIN?', 'gun', 'Not to front, Elvin is the kind of guy who could wear a bow tie and really pull it off. And don’t forget the middle initial MAD(D) Real. :100:'],
      ['WHO IS DIGGITY DOM?', 'gun', 'Not to front, he is master of QA aka Smooth criminal of code breaks. Get it right. :100:'],
      ['WHO IS FRANK?', 'gun', 'Not to front, Frank White was the King Of New York. Franco Jaramillo is the best producer since Dr Dre. :100:'],
      ['WHO IS JEFF?', 'troll', 'Jeff ain\'t nobody. He just Javascript baby daddy (in my Keisha from Belly voice). :100:']
    ]
    ...
    if (message.text.toUpperCase().indexOf('OH SHIT') > -1) {
      if (message.user === process.env.ADMIN6 || message.user === process.env.ADMIN1 || message.user === process.env.ADMIN5) {
        bot.reply(message, 'http://i.imgur.com/aMgG2jh.gif')
      }
    }
    if (message.text.toUpperCase().indexOf('FIGURE IT OUT') > -1) {
      if (message.user === process.env.ADMIN6 || message.user === process.env.ADMIN1) {
